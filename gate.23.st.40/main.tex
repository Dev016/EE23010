...
\begin{enumerate}[label = (\roman*)]
	\item (double) rand()/ RAND\_MAX: \\ Generates a uniform distribution between 0 and 1.
	\item sqrt(variance)*(sqrt(-2 * log(u1)) * cos (2 *M\_PI * u2)) + mean: \\ Transforms the uniform distribution into gaussian distribution of desired mean and variance. Ten such random variables are generated.
	\item The values of the random variables are squared and then added together to generate $T$.
